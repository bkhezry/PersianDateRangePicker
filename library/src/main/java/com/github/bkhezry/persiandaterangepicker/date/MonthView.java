...
public abstract class MonthView extends View {
  ...
  private final PersianDate mPersianDate;
  protected final PersianDate mDayLabelCalendar;
  ...
  public void setMonthParams(HashMap<String, Integer> params) {
    ...
    mPersianDate.setPersianDate(mYear, mMonth, 1);
    //TODO important
    mDayOfWeekStart = mPersianDate.get(Calendar.DAY_OF_WEEK);
  ...
  private boolean sameDay(int day, PersianDate today) {
  ...
  private String getMonthAndYearString() {
    ...
    return LanguageUtils.getPersianNumbers(
      mPersianDate.getPersianMonthName() + " " + mPersianDate.getPersianYear());
  }
  ...
  private boolean isSelectable(int year, int month, int day) {
    PersianDate[] selectableDays = mController.getSelectableDays();
    for (PersianDate persianDate : selectableDays) {
      if (year < persianDate.getPersianYear()) {
        break;
      }
      if (year > persianDate.getPersianYear()) {
        continue;
      }
      if (month < persianDate.getPersianMonth()) {
        break;
      }
      if (month > persianDate.getPersianMonth()) {
        continue;
      }
      if (day < persianDate.getPersianDay()) {
        break;
      }
      if (day > persianDate.getPersianDay()) {
        continue;
  ...
  private boolean isBeforeMin(int year, int month, int day) {
    ...
    PersianDate minDate = mController.getMinDate();
    if (minDate == null) {
      return false;
    }
  ...
  private boolean isAfterMax(int year, int month, int day) {
    ...
    PersianDate maxDate = mController.getMaxDate();
    if (maxDate == null) {
      return false;
    }

    if (year > maxDate.getPersianDay()) {
      return true;
  ...
  protected boolean isHighlighted(int year, int month, int day) {
    PersianDate[] highlightedDays = mController.getHighlightedDays();
    if (highlightedDays == null) {
      return false;
    }
    for (PersianDate c : highlightedDays) {
  ...
  protected class MonthViewTouchHelper extends ExploreByTouchHelper {
    ...
    private final PersianDate mTempCalendar = new PersianDate();
    ...
    protected CharSequence getItemDescription(int day) {
      ...
      final CharSequence date = LanguageUtils.getPersianNumbers(mTempCalendar.getPersianLongDateAndTime());
